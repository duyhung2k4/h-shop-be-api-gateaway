...
func Router() http.Handler {
	...
	app.Route("/", func(r chi.Router) {
		...
		r.Handle("/search/*", http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
			backendURL, _ := url.Parse(config.GetUrlSearchService())
			proxy := httputil.NewSingleHostReverseProxy(backendURL)
		...
		r.Handle("/payment/*", http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
			backendURL, _ := url.Parse(config.GetUrlPayment())
			proxy := httputil.NewSingleHostReverseProxy(backendURL)
